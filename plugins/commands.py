...
async def start(client, message):
    if message.chat.type in ['group', 'supergroup']:
        buttons = [
            [
                InlineKeyboardButton('💫 𝙐𝙋𝘿𝘼𝙏𝙀𝙎', url='https://t.me/Ak_Updates_botz')
            ],
            [
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton('ᴜᴘᴅᴀᴛᴇs', url=f'https://t.me/Ak_Updates_botz')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton(
                    " Join 𝙐𝙥𝙙𝙖𝙩𝙚𝙨 Channel", url=invite_link.invite_link
                )
            ]
        ]
        ...
        if message.command[1] != "subscribe":
            ...
            btn.append([InlineKeyboardButton("♻️ Try Again 👈 Tap me 🥰", callback_data=f"{pre}#{file_id}")])
